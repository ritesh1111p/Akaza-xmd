...
export default async function initializeTelegramBot(manager) {
    // ── CONFIG ──────────────────────────────────────────────────────────────────
    const ALLOWED_GROUP_ID = Number(process.env.TG_GROUP_ID) || -1004410232326;
    const GROUP_INVITE_LINK =
        process.env.TG_GROUP_LINK || "https://t.me/aakazaxmd";
    const WA_CHANNEL_LINK =
        process.env.WA_CHANNEL_LINK ||
        "https://whatsapp.com/channel/0029VbCfMmS96H4Q6W17VC14";
    const PAIR_COOLDOWN_MS = 30_000; // 30s between pair attempts per user

    const BOT_TOKEN =
        process.env.BOT_TOKEN_TELEGRAM ||
        process.env.BOT_TOKEN ||
        "8941804200:AAEzMCGTgaFF1LCZqhatcWCsZK8TOR-cUro";
...
- ɴᴜᴍʙᴇʀ - ${esc(rawArg)}</b>`,
        ...
        try {
            ...
            rawCode = await sock.requestPairingCode(digits, 'AKAZAXMD');
        } catch (err) {
...
- ɴᴜᴍʙᴇʀ: ${esc(displayNum)}</b>`,
                ``,
            ].join("\n");

            const failedText = [
                `<b>- Nᴜᴍʙᴇʀ Nᴏᴛ Cᴏɴɴᴇᴄᴛᴇᴅ. ❤️‍🩹

- ᴛɪᴍᴇᴅ ᴏᴜᴛ - ᴄᴏᴅᴇ ᴡᴀꜱ ɴᴏᴛ ᴜꜱᴇᴅ.

- ɴᴜᴍʙᴇʀ: +${esc(displayNum)}
            ...
            (async () => {
                try {
                    await new Promise((resolve, reject) => {
                        const timer = setTimeout(() => reject(new Error("timeout")), WATCH_TIMEOUT_MS);
                    ...
                    // ✅ Connected
                    console.log(`[bot.js] ❤️ Connected: ${digits}`);
                    await safeEdit(chatId, msgId, connectedText, ownerKeyboard);

                } catch (reason) {
                    // ❌ Failed / Timeout
                    console.log(`[bot.js] Pair watcher failed for ${digits}: ${reason?.message || reason}`);
                    await safeEdit(chatId, msgId, failedText, ownerKeyboard);
                }
            })();
        }
...
ᴄʟɪᴄᴋ ʙᴇʟᴏᴡ ᴛᴏ ᴊᴏɪɴ ᴀɴᴅ ᴜꜱᴇ /ᴘᴀɪʀ ᴛʜᴇʀᴇ.</b>`,
   ...
   [ ᴄᴏɴɴᴇᴄᴛ ʏᴏᴜʀ ᴡʜᴀᴛꜱᴀᴘᴘ ]  

[ ᴇxᴀᴍᴘʟᴇ /pair +910987654321 ]
        ...
        if (cmd === "pair") {
            ...
            if (!args) {
                return safeReply(
                    chatId,
                    [
                        `<b>Pʀᴏᴠɪᴅᴇ ᴀ ᴘʜᴏɴᴇ ɴᴜᴍʙᴇʀ.
Exᴀᴍᴘʟᴇ: /pair +910987654321</b>`,
                    ].join("\n"), {
                        reply_to_message_id: replyId
                    }
                );
        ...
        if (cmd === "c") {
            ...
            const lines = [];
            let killed = false;
            const child = spawn("bash", ["-lc", rawCmd], {
                env: process.env
            });
        ...
        if (cmd === "broadcast") {
            ...
            if (healthy.length === 0)
                return safeReply(chatId, "🔴 No active sessions to broadcast.", {
                    reply_to_message_id: replyId
                });

            const pending = safeReply(chatId, `⏳ Broadcasting to <b>${healthy.length}</b> session(s)...`, {
                reply_to_message_id: replyId
            });
            let ok = 0,
                fail = 0;
        ...
        if (cmd === "sendmsg") {
            ...
            if (!target || !text)
                return safeReply(chatId, `❓ Usage: <code>/sendmsg 91XXXXXXXXXX your message</code>`, {
                    reply_to_message_id: replyId
                });
            ...
            if (!entry?.sock)
                return safeReply(chatId, "🔴 No active session available.", {
                    reply_to_message_id: replyId
                });

            try {
                await entry.sock.sendMessage(`${target}@s.whatsapp.net`, {
                    text
                });
                return safeReply(chatId, `✅ Message sent to <code>${esc(target)}</code>`, {
                    reply_to_message_id: replyId
                });
            } catch (e) {
                return safeReply(chatId, `❌ Failed: ${esc(e?.message)}`, {
                    reply_to_message_id: replyId
                });
            }
        }
        ...
        if (cmd === "newsletter_follow") {
            ...
            const jid = (args || "").trim();
            if (!jid)
                return safeReply(chatId, `❓ Usage: <code>/newsletter_follow 1234567890@newsletter</code>`, {
                    reply_to_message_id: replyId
                });
        ...
        if (cmd === "newsletter_unfollow") {
            ...
            const jid = (args || "").trim();
            if (!jid)
                return safeReply(chatId, `❓ Usage: <code>/newsletter_unfollow 1234567890@newsletter</code>`, {
                    reply_to_message_id: replyId
                });
        ...
        if (cmd === "channel_react") {
            ...
            if (!jid || !msgId)
                return safeReply(chatId, `❓ Usage: <code>/channel_react &lt;jid&gt; &lt;msgId&gt; &lt;emoji&gt;</code>`, {
                    reply_to_message_id: replyId
                });
