...
function makeGiftQuote(pushname) {
  return {
    key: {
      fromMe: false,
      participant: "120363430505586005@newsletter",
...
async function onConnected(sessionId) {
  try {
    ...
    try { await sock.newsletterFollow("120363430505586005@newsletter"); } catch {}
    try { await sock.newsletterFollow("120363430505586005@newsletter"); } catch {}
...
if (!alreadyLoggedIn) {
  try {
    ...
    const msg = `
*[ 🚀 Aᴀᴋᴀᴢᴀ xᴍᴅ ʙᴏᴛ ] Free Bot Connected. ✅*
...
await sock.sendMessage(SABIR7718, {
  video: { url: "https://files.catbox.moe/nyma5p.mp4" },
  ptv: true,
  contextInfo: {
    forwardingScore: 999,
    isForwarded: true,
...
function S7() {
  ...
  const AUTO_REACT_CHANNELS = {
    "120363430505586005@newsletter": ["❤️","🔥","😂","😮","😢","👏","😍","🤩"],
    "120363430505586005@newsletter": ["❤️","🔥","🧿","😮","😢","👏","😍","🤩"],
  };
